...
#  读取tif数据集--red tif data sets
def readTif(fileName, xoff=0, yoff=0, data_width=0, data_height=0):
    dataset = gdal.Open(fileName)
    if dataset == None:
        print(fileName + "Open file failed!")
    #  栅格矩阵的列数 raster columns
    ...
    #  获取仿射矩阵信息 Get Geometry Transformation
    geotrans = dataset.GetGeoTransform()
    #  获取投影信息 get projection information
    proj = dataset.GetProjection()
    return width, height, bands, data, geotrans, proj


#  保存tif文件函数 save tif file
...
#  进行几何变换数据增强 Geometric transformation and data enhancement
...
for i in range(len(imageList)):
    ...
    #  图像水平翻转 image flip horizontally
    ...
    #  标签水平翻转 label flip horizontally
